        ...
        for i in range(100):
             with Profiler("Environment action step"):
                if 'w' in pressed_keys:
                    ...
                    action = [-1.0, 0.0, 0.0]  # Move backward
                elif 'a' in pressed_keys:
                    action = [0.0, 0.0, 1.0]  # Turn left
                elif 'd' in pressed_keys:
                    action = [0.0, 0.0, -1.0]  # Turn right
                else:
                    action = [0.0, 0.0, 0.0] * action_space_dimension  # No action

                # Ensure the action dimension matches the robot's action space
                assert len(action) == action_space_dimension, "Action dimension mismatch"
